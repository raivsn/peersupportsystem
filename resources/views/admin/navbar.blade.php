<nav class="navbar">
    <div class="header-content" style="display: flex; align-items: center; justify-content: space-between; width: 100%; gap: 1.5rem;">
        <div class="left-section" style="display: flex; align-items: center; gap: 0.7rem;">
            <a href="https://www.nasom.org.my" target="_blank" style="text-decoration: none; display: flex; align-items: center; gap: 0.7rem;">
                <img src="/nasom-logo.png" alt="NASOM Logo" class="logo-img">
                <span class="logo-text">PeerCare Admin</span>
            </a>
        </div>
        <div style="display: flex; align-items: center; gap: 2.2rem;">
            <div class="nav-links" style="display: flex; gap: 1.5rem; align-items: center;">
                <a href="{{ route('dashboard.admin') }}" style="font-weight:{{ $active == 'dashboard' ? 'bold' : 'normal' }}; color:{{ $active == 'dashboard' ? '#3b3b6d' : 'inherit' }};">Dashboard</a>
                <a href="{{ route('forum.home') }}" style="font-weight:{{ $active == 'forum' ? 'bold' : 'normal' }}; color:{{ $active == 'forum' ? '#3b3b6d' : 'inherit' }};">Forum</a>
                <a href="{{ route('admin.resources') }}" style="font-weight:{{ $active == 'resources' ? 'bold' : 'normal' }}; color:{{ $active == 'resources' ? '#3b3b6d' : 'inherit' }};">Resource Library Management</a>
                <a href="{{ route('admin.feedback') }}" style="font-weight:{{ $active == 'feedback' ? 'bold' : 'normal' }}; color:{{ $active == 'feedback' ? '#3b3b6d' : 'inherit' }};">Feedback Viewer</a>
                <a href="{{ route('admin.profanity') }}" style="font-weight:{{ $active == 'profanity' ? 'bold' : 'normal' }}; color:{{ $active == 'profanity' ? '#3b3b6d' : 'inherit' }};">Profanity Filter</a>
            </div>
            <div class="user-menu" id="user-menu">
                ...
                <div class="dropdown">
                    <a href="{{ route('admin.profile') }}" style="font-weight:{{ $active == 'profile' ? 'bold' : 'normal' }}; color:{{ $active == 'profile' ? '#3b3b6d' : 'inherit' }};">Manage Profile</a>
